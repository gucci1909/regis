...
router.post(
  '/individual-domestic',
  upload.fields([
    { name: 'emiratesIdUpload', maxCount: 1 },
    { name: 'passportUpload', maxCount: 1 },
    { name: 'reraIdUpload', maxCount: 1 },
  ...
  async (req, res) => {
    try {
      ...
      const fileFields = ['emiratesIdUpload', 'passportUpload', 'reraIdUpload', 'cvUpload'];
      ...
      const [emiratesIdUpload, passportUpload, reraIdUpload, cvUpload] =
        await Promise.all(uploadPromises);

      // Save company data to MongoDB
      const db = getDb();
      const collection = db.collection('request-individual-domestic-companies');
...
router.get('/individual-domestic', async (req, res) => {
  try {
    const db = getDb();
    const collection = db.collection('request-individual-domestic-companies');
...
router.post('/individual-domestic/status-change', async (req, res) => {
  try {
